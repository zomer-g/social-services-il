-- rebuild_cards, redefined so that a card carries the freshness of the record
-- behind it and not the moment the rebuild ran, and so that placeholder phone
-- numbers and city names never reach a card.
--
-- Everything else is unchanged from 005; the whole function is restated because
-- Postgres has no way to patch part of one.
...
BEGIN
  ...
  assembled AS (
    SELECT
      ...
      -- When the record itself last changed, as far as anyone can tell: what the
      -- source said, else when we last wrote it. Never the rebuild time.
      GREATEST(
        COALESCE(s.source_updated_at, s.updated_at),
        COALESCE(b.source_updated_at, b.updated_at, to_timestamp(0)),
